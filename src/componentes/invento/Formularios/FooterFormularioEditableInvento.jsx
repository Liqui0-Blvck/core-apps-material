...
const FooterFormularioEditableInvento = ({ isValid, rows, setRows, handleAgregarItem }) => {
  ...
  const agregarFila = () => {
    const nuevaFila = { id: rows.id, item: '', cantidad: 0 };
  ...
  const handleChangeRow = (id, fieldName, value) => {
    isValid(fieldName === "cantidad" && parseInt(value) > 0 ? true : false);
    setRows((prevRows) =>
        ...
        <Table sx={{ minWidth: 750, background: 'white' }} aria-label="simple table">
          ...
          <TableBody>
            {rows && rows.map((row, index) => {
              ...
              return (<TableRow key={index} style={{ background: 'white', margin: 'auto' }}>
                <TableCell component="th" scope="row" style={{ maxWidth: 200, minWidth: 200 }}>
                  <Select
                    ...
                    onChange={(value) => {
                      handleChangeRow(row.id, "item", value)
                    }}
                    onSearch={onSearch}
                    name='item'
                    value={itemNombre && itemNombre.find(item => item.value === row.item)?.label || ""}
...
export default FooterFormularioEditableInvento;
